...
MecanumSerialPort::MecanumSerialPort()
    : serial_port_(-1)
    , rx_frame_length_(0)
...
return_type MecanumSerialPort::open(const std::string & port_name)
{
    serial_port_ = ::open(port_name.c_str(), O_RDWR | O_NOCTTY);

    if (serial_port_ < 0) {
        RCLCPP_ERROR(rclcpp::get_logger("MecanumSerialPort"), "Failed to open serial port: %s (%d)", strerror(errno), errno);
        return return_type::ERROR;
    }

    struct termios tty_config{};
    if (::tcgetattr(serial_port_, &tty_config) != 0) {
        RCLCPP_ERROR(rclcpp::get_logger("MecanumSerialPort"), "Failed to get serial port configuration: %s (%d)", strerror(errno), errno);
        close();
        return return_type::ERROR;
    }
    ...
    */
    ...
    if (::tcsetattr(serial_port_, TCSANOW, &tty_config) != 0) {
        RCLCPP_ERROR(rclcpp::get_logger("MecanumSerialPort"), "Failed to set serial port configuration: %s (%d)", strerror(errno), errno);
        close();
        return return_type::ERROR;
    }
...
return_type MecanumSerialPort::close()
{
    if (is_open()) {
        ::close(serial_port_);
        serial_port_ = -1;
...
return_type MecanumSerialPort::read_frames(std::vector<SerialHdlcFrame>& frames)
{
    // Read data from the serial port
    const ssize_t num_bytes = ::read(serial_port_, rx_buffer_, 256);
    if (num_bytes == -1) {
        RCLCPP_ERROR(rclcpp::get_logger("MecanumSerialPort"), "Failed to read serial port data: %s (%d)", strerror(errno), errno);
        return return_type::ERROR;
    }
...
return_type MecanumSerialPort::write_frame(const uint8_t* data, size_t size)
{
    if (!is_open()) {
        return return_type::ERROR;
    }
    ...
    if (::write(serial_port_, tx_frame_buffer_, tx_frame_length_) == -1) {
        RCLCPP_ERROR(rclcpp::get_logger("MecanumSerialPort"), "Failed to write serial port data: %s (%d)", strerror(errno), errno);
        return return_type::ERROR;
    }
...
bool MecanumSerialPort::is_open() const
{
    return serial_port_ >= 0;
}

void MecanumSerialPort::encode_byte(uint8_t data)
{
...
void MecanumSerialPort::decode_byte(uint8_t data, std::vector<SerialHdlcFrame>& frames)
{
    ...
    if (rx_frame_length_ == MECANUM_SERIAL_SERIAL_FRAME_MAX_SIZE) {
        rx_frame_length_ = 0;
        rx_frame_crc_ = HDLC_CRC_INIT_VALUE;
    }
}

uint16_t MecanumSerialPort::crc_update(uint16_t crc, uint8_t data)
{
