...
int main(int argc, char * argv[])
{
    ...
    std::thread cm_thread([controller_manager_node]() {
        ...
        std::chrono::time_point<std::chrono::system_clock, std::chrono::nanoseconds> next_interation_time{cm_now};
        ...
        while (rclcpp::ok()){
            ...
            next_interation_time += period;
            std::this_thread::sleep_until(next_interation_time);
        }
    });
    ...
    controller_manager_node->load_controller("mecanum_controller", "mecanum_controller/MecanumController");
    controller_manager_node->configure_controller("joint_state_broadcaster");
    controller_manager_node->configure_controller("mecanum_controller");

    start_controllers.push_back("joint_state_broadcaster");
    start_controllers.push_back("mecanum_controller");
